...
L<create a token|https://github.com/settings/tokens>. Only the C<gist>
...
L<https://gist.github.com/5590bc07b8d5bd8fd98d>).
...
values C<content> set to content of files to paste. Using the hashref
allows you to make a gist with multiple files. All other arguments
are are options and are as follows:
...
B<Optional>. Takes C<True> or C<False> values. If set to true, your
...
B<Optional>. Applies only when the first positional argument to C</paste> is
a string. Specifies the filename to use for your gist (affects syntax
highlighting). B<Defaults to:> C<nopaste.txt>.
...
It tells you not to use files C<gistfile3> or C<gistfiles33.txt>. Behaviour
